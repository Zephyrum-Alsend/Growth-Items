...
XP acquisition is handled by an OBSE OnHitWith handler, looking for the specific version of Shurifen you
should have at its current level.  Upon leveling, the script will check 4 things: that you don't have the
max level variant; the level up flag is set; you aren't in combat or riding; you have exactly 1 instance of
a version of Shurifen.  This is why taking one out of the cheat chest will break the script.

XP required per level is determined by the formula:
BaseRequirement + ( aaShurifenLevel * LevelMult )
Where BaseRequirement = 200 and LevelMult = 200.  aaShurifenLevel is incremented every time Shurifen 
levels.  The starting XP requirement is 400 (200 + ( 1 * 200 )).
